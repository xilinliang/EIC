...

{
  ...
  plots->h1Pi_matchTrkE = result->AddHist1D("h1Pi_matchTrkE","match Pion track energy","E [GeV]","events",100,0.,10.);
  plots->h1Pi_nomatchTrkZ = result->AddHist1D("h1Pi_nomatchTrkZ","no match Pion track fraction z","z","events",20,0,1.);
  plots->h1Pi_nomatchTrkE = result->AddHist1D("h1Pi_nomatchTrkE","no match Pion track energy","E [GeV]","events",100,0.,10.);
  ...
  plots->h1K_matchTrkE = result->AddHist1D("h1K_matchTrkE","match Kaon track energy","E [GeV]","events",100,0.,10.);
  plots->h1K_nomatchTrkZ = result->AddHist1D("h1K_nomatchTrkZ","no match Kaon track fraction z","z","events",20,0,1.);
  plots->h1K_nomatchTrkE = result->AddHist1D("h1K_nomatchTrkE","no match Kaon track energy","E [GeV]","events",100,0.,10.);
  ...
  plots->h1Pr_matchTrkE = result->AddHist1D("h1Pr_matchTrkE","match Pion track energy","E [GeV]","events",100,0.,10.);
  plots->h1Pr_nomatchTrkZ = result->AddHist1D("h1Pr_nomatchTrkZ","no match Proton track fraction z","z","events",20,0,1.);
  plots->h1Pr_nomatchTrkE = result->AddHist1D("h1Pr_nomatchTrkE","no match Proton track energy","E [GeV]","events",100,0.,10.);
  ...
  plots->h1E_matchTrkE = result->AddHist1D("h1E_matchTrkE","match electron track energy","E [GeV]","events",100,0.,10.);
  plots->h1E_nomatchTrkZ = result->AddHist1D("h1E_nomatchTrkZ","no match electron track fraction z","z","events",20,0,1.);
  plots->h1E_nomatchTrkE = result->AddHist1D("h1E_nomatchTrkE","no match electron track energy","E [GeV]","events",100,0.,10.);
...

{
  ...
  TClonesArray *branchmRICHTrack = treeReader->UseBranch("mRICHTrack"); 
...
//      true_tracks.push_back(trk_in);
    ...
    for (int i = 0; i < branchmRICHTrack->GetEntries(); ++i) {
      mRICHTrack = (Track*)branchmRICHTrack->At(i);

      if (mRICHTrack->PT < 0.1 || TMath::Abs(mRICHTrack->Eta) > 3.5) continue;

	PIDnum = mRICHTrack->PID;
	...
	int totalE=0; matchE_PID = 0; E_purity = -1;
...

      {
        ...
        // Check if the constituent is accessible
			...
			if ((track->P4().Eta() < -3.5)||(track->P4().Eta()>-1)) continue;
			...
			if (abs(track->PID) == 211)
			{
				...
				for (int PIDi=0;PIDi < nPi_PID;++PIDi)
				{
					...
					mRICHTrack = (Track*)branchmRICHTrack->At(Pi_PID[PIDi]);
					if (track->Particle == mRICHTrack->Particle) {isMatch = 1;++matchPi_PID; h1Pi_matchTrkZ->Fill(z); h1Pi_matchTrkE->Fill(track->P4().E());}
				}
			}
			if ((abs(track->PID) == 211) && (isMatch != 1)) {h1Pi_nomatchTrkZ->Fill(z); h1Pi_nomatchTrkE->Fill(track->P4().E());}
			...
			if (abs(track->PID) == 321)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nK_PID;++PIDi)
	                        {
	                                ...
	                                mRICHTrack = (Track*)branchmRICHTrack->At(K_PID[PIDi]);
	                                if (track->Particle == mRICHTrack->Particle) {isMatch = 1;++matchK_PID; h1K_matchTrkZ->Fill(z); h1K_matchTrkE->Fill(track->P4().E());}
	                        }
	                }
			...
			if (abs(track->PID) == 2212)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nPr_PID;++PIDi)
	                        {
	                                ...
	                                mRICHTrack = (Track*)branchmRICHTrack->At(Pr_PID[PIDi]);
	                                if (track->Particle == mRICHTrack->Particle) {isMatch = 1; ++matchPr_PID; h1Pr_matchTrkZ->Fill(z); h1Pr_matchTrkE->Fill(track->P4().E());}
			...
			if (abs(track->PID) == 11)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nE_PID;++PIDi)
	                        {
	                                ...
	                                mRICHTrack = (Track*)branchmRICHTrack->At(E_PID[PIDi]);
	                                if (track->Particle == mRICHTrack->Particle) {isMatch =1; ++matchE_PID; h1E_matchTrkZ->Fill(z); h1E_matchTrkE->Fill(track->P4().E());}
...
void PID_mRICHTrack(const char *inputFile)
