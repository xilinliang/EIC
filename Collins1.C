...

{
  ...
  TH1 *res_dphi_diffz_10_20[18];

  TH1 *res_dphi_diffz_20_30[18];

  TH1 *res_dphi_diffz_30_40[18];

  TH1 *res_dphi_diffz_40_50[18];
...

{
  ...
  for (int i=0;i<16;i=i+1)
  {
	...
	sprintf(name_hist,"dphi_z_%i_%i_1020",i*5,(i+1)*5);
	plots->res_dphi_diffz_10_20[i] = result->AddHist1D(name_hist,name_hist,"#phi_{C}^{gen}-#phi_{C}^{reco} [rad]","event",100,-TMath::Pi(),TMath::Pi());
	sprintf(name_hist,"dphi_z_%i_%i_2030",i*5,(i+1)*5);
        plots->res_dphi_diffz_20_30[i] = result->AddHist1D(name_hist,name_hist,"#phi_{C}^{gen}-#phi_{C}^{reco} [rad]","event",100,-TMath::Pi(),TMath::Pi());
	sprintf(name_hist,"dphi_z_%i_%i_3040",i*5,(i+1)*5);
        plots->res_dphi_diffz_30_40[i] = result->AddHist1D(name_hist,name_hist,"#phi_{C}^{gen}-#phi_{C}^{reco} [rad]","event",100,-TMath::Pi(),TMath::Pi());
	sprintf(name_hist,"dphi_z_%i_%i_4050",i*5,(i+1)*5);
        plots->res_dphi_diffz_40_50[i] = result->AddHist1D(name_hist,name_hist,"#phi_{C}^{gen}-#phi_{C}^{reco} [rad]","event",100,-TMath::Pi(),TMath::Pi());
...
//	 cout<<"(genjet->P4().E()="<<genjet->P4().E()<<endl;
	  ...
	  if((genjet->P4().E())>10 && (genjet->P4().E())<20){
		...
		if ((floor(genz * 20) < 16) && (floor(genz * 20) >=0)) 
		{
		  cout<<(floor(genz * 20))<<endl;
		  plots->res_dphi_diffz_10_20[floor(genz * 20)]->Fill(dphi);
		};
	  }

	  else if((genjet->P4().E())>20 && (genjet->P4().E())<40){

	    plots->res_dphi_z_20_40->Fill(dphi, genz);

	    if (floor(genz * 20) < 16 && (floor(genz * 20) >=0))
	    {
		cout<<(floor(genz * 20))<<" "<<genjet->P4().E()<<endl;
		if ((genjet->P4().E())>30) {plots->res_dphi_diffz_30_40[floor(genz * 20)]->Fill(dphi);
		}
		else {plots->res_dphi_diffz_20_30[(floor(genz * 20))]->Fill(dphi);}
	    }

	  }

	  else if((genjet->P4().E())>40 && (genjet->P4().E())<60){

	    plots->res_dphi_z_40_60->Fill(dphi, genz);

	    if ((genjet->P4().E())<50 && (floor(genz * 20) < 16) && (floor(genz * 20) >=0)) {
		cout<<(floor(genz * 20))<<endl;
		plots->res_dphi_diffz_40_50[floor(genz * 20)]->Fill(dphi);}
	  ...
	  else if((genjet->P4().E())>60 && (genjet->P4().E())<80){
...
void Collins1(const char *inputFile)
