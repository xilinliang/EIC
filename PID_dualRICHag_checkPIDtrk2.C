...

{
  ...
  TLorentzVector momentum, InPr , InE, OutE, dualRICHagTrackP4;
...
//    for(entry = 0; entry < 5; ++entry)
  {
   ...
   // nE_true=0;
	...
	for (int j=0; j < branchdualRICHagTrack->GetEntries(); ++j)
	{

		dualRICHagTrack = (Track*)branchdualRICHagTrack->At(j);
		dualRICHagTrackP4 = dualRICHagTrack->P4();
		dualRICHagTrackZ = (InPr.Dot(dualRICHagTrackP4)) / (InPr.Dot(InE - OutE));
		particle = (GenParticle*) dualRICHagTrack->Particle.GetObject();
		if (abs(particle->PID) == abs(dualRICHagTrack->PID)) {checkmatch = 1;} else {checkmatch = -1;}
		//cout<<"check match="<<checkmatch<<endl;
		//cout<<"true track PID:"<<abs(particle->PID)<<"  PID track PID:"<<abs(dualRICHagTrack->PID)<<endl;
		PIDnum = abs(particle->PID);
		PIDtrknum = abs(dualRICHagTrack->PID);


		if (checkmatch == -1) 
		{
			h1checknomatchTrkE->Fill(dualRICHagTrack->P4().E());
			if (abs(PIDnum) == 211) {h1Pi_checknomatchTrkZ->Fill(dualRICHagTrackZ); h1Pi_checknomatchTrkE->Fill(dualRICHagTrack->P4().E());} 
			if (abs(PIDnum) == 321) {h1K_checknomatchTrkZ->Fill(dualRICHagTrackZ); h1K_checknomatchTrkE->Fill(dualRICHagTrack->P4().E());} 
			if (abs(PIDnum) == 2212) {h1Pr_checknomatchTrkZ->Fill(dualRICHagTrackZ); h1Pr_checknomatchTrkE->Fill(dualRICHagTrack->P4().E());} 
		}

		if (checkmatch == 1) 
		{
			h1checkmatchTrkE->Fill(dualRICHagTrack->P4().E());
			if (abs(PIDnum) == 211) {h1Pi_checkmatchTrkZ->Fill(dualRICHagTrackZ); h1Pi_checkmatchTrkE->Fill(dualRICHagTrack->P4().E());} 
			if (abs(PIDnum) == 321) {h1K_checkmatchTrkZ->Fill(dualRICHagTrackZ); h1K_checkmatchTrkE->Fill(dualRICHagTrack->P4().E());} 
			if (abs(PIDnum) == 2212) {h1Pr_checkmatchTrkZ->Fill(dualRICHagTrackZ); h1Pr_checkmatchTrkE->Fill(dualRICHagTrack->P4().E());} 
		}
...
//		if (SmearingTrack->Particle.GetObject() == dualRICHagTrack->Particle.GetObject()) {checkmatch = 1;}
//		j=j+1;		
//
//	}
...
void PID_dualRICHag_checkPIDtrk2(const char *inputFile)
