        ...
        // Check if the constituent is accessible
			...
			if (abs(track->PID) == 211)
			{
				...
				for (int PIDi=0;PIDi < nPi_PID;++PIDi)
				{
					...
					if (track->Particle == barrelDIRCTrack->Particle) {isMatch = 1;++matchPi_PID; h1Pi_matchTrkZ->Fill(z); h1Pi_matchTrkE->Fill(track->P4().E());}
			...
			if (abs(track->PID) == 321)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nK_PID;++PIDi)
	                        {
	                                ...
	                                if (track->Particle == barrelDIRCTrack->Particle) {isMatch = 1;++matchK_PID; h1K_matchTrkZ->Fill(z); h1K_matchTrkE->Fill(track->P4().E());}
					if (track->P4().E() == barrelDIRCTrack->P4().E()) 
					{
			...
			if (abs(track->PID) == 2212)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nPr_PID;++PIDi)
	                        {
	                                ...
	                                if (track->Particle == barrelDIRCTrack->Particle) {isMatch = 1; ++matchPr_PID; h1Pr_matchTrkZ->Fill(z); h1Pr_matchTrkE->Fill(track->P4().E());}
					if (track->P4().E() == barrelDIRCTrack->P4().E()) 
					{
			...
			if (abs(track->PID) == 11)
	                {
	                        ...
	                        for (int PIDi=0;PIDi < nE_PID;++PIDi)
	                        {
	                                ...
	                                if (track->Particle == barrelDIRCTrack->Particle) {isMatch =1; ++matchE_PID; h1E_matchTrkZ->Fill(z); h1E_matchTrkE->Fill(track->P4().E());}
