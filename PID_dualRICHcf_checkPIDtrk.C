...
//	cout<<"energy fraction z:"<<SmearingTrackZ<<endl;
	...
	while (j < branchdualRICHcfTrack->GetEntries() && checkmatch == 0)
	{
		...
		dualRICHcfTrack = (Track*)branchdualRICHcfTrack->At(j);
			...
			if (SmearingTrack->Particle.GetObject() == dualRICHcfTrack->Particle.GetObject()) 
			{
				checkmatch = -1;
				if (SmearingTrack->PID == dualRICHcfTrack->PID) {checkmatch = 1;}
...
void PID_dualRICHcf_checkPIDtrk(const char *inputFile)
