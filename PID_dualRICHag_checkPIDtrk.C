...
//	cout<<"energy fraction z:"<<SmearingTrackZ<<endl;
	...
	while (j < branchdualRICHagTrack->GetEntries() && checkmatch == 0)
	{
		...
		dualRICHagTrack = (Track*)branchdualRICHagTrack->At(j);
			...
			if (SmearingTrack->Particle.GetObject() == dualRICHagTrack->Particle.GetObject()) 
			{
				checkmatch = -1;
				if (SmearingTrack->PID == dualRICHagTrack->PID) {checkmatch = 1;}
...
void PID_dualRICHag_checkPIDtrk(const char *inputFile)
