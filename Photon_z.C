...

{
  ...
  plots->h1PhotonZ = result->AddHist1D("h1PhotonZ","EFlow Photon energy fraction z","z","events",20,0.,1.);
  plots->h1PhotonE = result->AddHist1D("h1PhotonE","EFlow Photon energy","E [GeV]","events",100,0,10.);
...

{
  ...
  TLorentzVector momentum, InPr , InE, OutE, EFlowPhotoni;
...
//    }
    ...
    for (int i = 0; i < branchEFlowPhoton->GetEntries(); ++i) 
   {
	...
	EFlowPhotoni = tower->P4();

	//cout<<"Photon E"<<EFlowPhotoni.E()<<endl;

	EFlowPhotonZ = (InPr.Dot(EFlowPhotoni)) / (InPr.Dot(InE - OutE));

	h1PhotonZ->Fill(EFlowPhotonZ);
	h1PhotonE->Fill(EFlowPhotoni.E());
...
void Photon_z(const char *inputFile)
